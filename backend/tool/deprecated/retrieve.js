...
async function main() {
  ...
  try {
    ...
    const issuerBalances = await xrplTool.getBalances(
      client,
    ...
    const issuerTrustLines = await xrplTool.getTrustLines(
      client,
      issuerWallet.address
    );
    ...
    issuerTrustLines.forEach((line) => {
      console.log(
        `  ${line.currency}: limit=${line.limit}, balance=${line.balance}`
      );
    });
    ...
    const treasuryBalances = await xrplTool.getBalances(
      client,
    ...
    const treasuryTrustLines = await xrplTool.getTrustLines(
      client,
      treasuryWallet.address
    );
    console.log("\nTrust Lines:");
    treasuryTrustLines.forEach((line) => {
      console.log(
        `  ${line.currency}: limit=${line.limit}, balance=${line.balance}, issuer=${line.account}`
      );
    });
    ...
    if (walletInfo.buyers && walletInfo.buyers.length > 0) {
      ...
      for (const buyerInfo of walletInfo.buyers) {
        ...
        const buyerBalances = await xrplTool.getBalances(
          client,
          buyerWallet.address
        );
        console.log("\nBalances:");
        buyerBalances.forEach((bal) => {
          const issuerLabel = bal.issuer
            ? `(issuer: ${bal.issuer})`
            : "(native)";
          console.log(`  ${bal.value} ${bal.currency} ${issuerLabel}`);
        });

        const buyerTrustLines = await xrplTool.getTrustLines(
          client,
          buyerWallet.address
        );
        console.log("\nTrust Lines:");
        ...
        const buyerTokenBalance = buyerBalances.find(
          (b) =>
            b.currency === walletInfo.bond.currencyCode &&
            b.issuer === issuerWallet.address
        );
    ...
    console.log(
      `Tokens in Circulation: ${walletInfo.bond.totalTokens - treasuryHoldings}`
    );
    console.log(
      `Number of Buyers: ${walletInfo.buyers ? walletInfo.buyers.length : 0}`
    );

    // Explorer links
    console.log("\n[XRPL Testnet Explorer]");
    console.log(
      `Issuer: https://testnet.xrpl.org/accounts/${issuerWallet.address}`
    );
    console.log(
      `Treasury: https://testnet.xrpl.org/accounts/${treasuryWallet.address}`
    );
    if (walletInfo.buyers && walletInfo.buyers.length > 0) {
      walletInfo.buyers.forEach((buyer) => {
        console.log(
          `${buyer.id}: https://testnet.xrpl.org/accounts/${buyer.address}`
        );
      });
    }
  } catch (error) {
