 ...
 * Force overwrite: node src/app/tool/issuer.js --force
 */
...
async function confirmOverwrite() {
  ...
  return new Promise((resolve) => {
    rl.question(
      "bond_wallets.json already exists. Overwrite? (y/N): ",
      (answer) => {
        rl.close();
...
async function main() {
  ...
  // Check if bond_wallets.json already exists
  const walletPath = path.join(__dirname, "bond_wallets.json");
  const forceOverwrite = process.argv.includes("--force");

  if (fs.existsSync(walletPath) && !forceOverwrite) {
    console.log("\nWARNING: bond_wallets.json already exists!");
    console.log("Running this script will overwrite existing wallet data.");
  ...
  try {
    ...
    treasuryBalances.forEach((bal) => {
      console.log(
        `  ${bal.value} ${bal.currency} (issuer: ${bal.issuer || "XRP"})`
      );
    });
    ...
    console.log(
      `Treasury: https://testnet.xrpl.org/accounts/${treasury.address}`
    );
    console.log("=".repeat(60));
  } catch (error) {
